 ...
 */
struct ValueRangePropagator(T = uint) if (is(T == uint) || is(T == ulong)) {
	...
	bool canFit(Expression e, BuiltinType t) in {
		...
		auto s = max(t.getSize(), uint.sizeof);
		if (s == T.sizeof) {
			auto r = visit(e);
		}
		...
		switch (s) {
			case uint.sizeof :
				return ValueRangePropagator!uint(pass).canFit(e, t);
			
			case ulong.sizeof :
				return ValueRangePropagator!ulong(pass).canFit(e, t);
	...
	auto processExpr(alias handler = asHandlerDMDMonkeyDance, A...)(Expression e, A args) {
		import std.algorithm;
	...
	VR visit(CastExpression e) {
		final switch (e.kind) with(CastKind) {
			...
			case Trunc :
				...
				return processExpr!doTheDMDMonkeyDance(e.expr, getBuiltin(e.type));
...
unittest {
	...
	foreach (T; AliasSeq!(uint, ulong)) {
		 ...
		 * Binary ops
		 */
		v = vrp.visit(new BinaryExpression(Location.init, tint, BinaryOp.Comma, i1, i2));
		assert(v == VR(42));
		
		// Technically, this is illegal, but it is out of scope of VRP to detect this, so will do.
		v = vrp.visit(new BinaryExpression(Location.init, tint, BinaryOp.Assign, i1, i2));
		assert(v == VR(42));
		
		v = vrp.visit(new BinaryExpression(Location.init, tint, BinaryOp.Add, i1, i2));
		assert(v == VR(35));
		
		v = vrp.visit(new BinaryExpression(Location.init, tint, BinaryOp.Sub, i1, i2));
		assert(v == VR(-49));
		
		v = vrp.visit(new BinaryExpression(Location.init, tint, BinaryOp.Mul, i1, i2));
		assert(v == VR(-294));
		
		v = vrp.visit(new BinaryExpression(Location.init, tuint, BinaryOp.UDiv, i1, i2));
		assert(v == VR((T.max - 6) / 42));
		
		v = vrp.visit(new BinaryExpression(Location.init, tint, BinaryOp.SDiv, i2, i1));
		assert(v == VR(-6));
		
		v = vrp.visit(new BinaryExpression(Location.init, tuint, BinaryOp.URem, i1, i2));
		assert(v == VR(LS ? 9 : 39));
		
		v = vrp.visit(new BinaryExpression(Location.init, tint, BinaryOp.SRem, i1, i3));
		assert(v == VR(-1));
		
		v = vrp.visit(new BinaryExpression(Location.init, tint, BinaryOp.Or, i1, i2));
		assert(v == VR(-5));
		
		v = vrp.visit(new BinaryExpression(Location.init, tint, BinaryOp.And, i1, i2));
		assert(v == VR(40));
		
		v = vrp.visit(new BinaryExpression(Location.init, tint, BinaryOp.Xor, i1, i2));
		assert(v == VR(-45));
		
		v = vrp.visit(new BinaryExpression(Location.init, tint, BinaryOp.LeftShift, i1, i3));
		assert(v == VR(-28));
		
		v = vrp.visit(new BinaryExpression(Location.init, tuint, BinaryOp.UnsignedRightShift, i1, i3));
		assert(v == VR((T.max >> 2) - 1));
		
		v = vrp.visit(new BinaryExpression(Location.init, tint, BinaryOp.SignedRightShift, i1, i3));
		assert(v == VR(-2));
		
		/**
		 * Unary ops
		 */
		v = vrp.visit(new UnaryExpression(Location.init, tuint, UnaryOp.Plus, i1));
		assert(v == VR(-7));
		
		v = vrp.visit(new UnaryExpression(Location.init, tuint, UnaryOp.Minus, i1));
		assert(v == VR(7));
		 ...
		 * Casts
		 */
		v = vrp.visit(new CastExpression(Location.init, CastKind.IntToBool, tbool, zero));
		assert(v == VR(0));
		
		v = vrp.visit(new CastExpression(Location.init, CastKind.IntToBool, tbool, i1));
		assert(v == VR(1));
		
		// FIXME: test the 0, 1 int to bool case ?
		
		v = vrp.visit(new CastExpression(Location.init, CastKind.Trunc, tubyte, i1));
		assert(v == VR(249));
		
		// XXX: These are shit tests :/
		v = vrp.visit(new CastExpression(Location.init, CastKind.UPad, tint, bmax));
		assert(v == VR(255));
		
		v = vrp.visit(new CastExpression(Location.init, CastKind.SPad, tint, bmax));
		assert(v == VR(-1));
		
		v = vrp.visit(new CastExpression(Location.init, CastKind.Bit, tint, i1));
		assert(v == VR(-7));
		
		v = vrp.visit(new CastExpression(Location.init, CastKind.Qual, tint, i1));
		assert(v == VR(-7));
		
		v = vrp.visit(new CastExpression(Location.init, CastKind.Exact, tint, i1));
		assert(v == VR(-7));
	}
}
...
struct ValueRange(T) if(is(uint : T) && isIntegral!T) {
	...
	ValueRange trunc(U mask) const out(result) {
		...
		auto fail = isSigned!T
			? ValueRange(~smask, smask)
			: ValueRange(0, mask);
	...
	auto opBinary(string op : "+")(ValueRange rhs) const {
		...
		// If the type is small enough, do the eay dance.
		auto overflow = Bits < 64
			? range > U.max
			: range < lrange && range < rrange;
	...
	ValueRange opBinary(string op : "*")(ValueRange rhs) const if (isUnsigned!T) {
	...
	auto opBinary(string op : "/")(ValueRange rhs) const {
		...
		if (isUnsigned!T) {
			...
			if (rhs.containsZero) {
				import std.algorithm : min, max;
				return ValueRange(min(lhs.min, -lhs.max), max(lhs.max, -lhs.min));
			}
	...
	ValueRange reduceOrdered(alias doOp, bool hasFlipped = false)(
		...
		while (true) {
			...
			static if (hasFlipped) {
				...
				result = result.unionWith(rhs.reduceOrdered!(doOp, true)(reduced));
			}
...
unittest {
	...
	foreach (T; AliasSeq!(int, uint, long, ulong)) {
		...
		assert(VR(31, smax + 31).normalized == (isSigned!T ? VR() : VR(31, smax + 31)));
		...
		// Do not fit
		assert(VR(5, 300).trunc(0xFF) == (isSigned!T ? VR(-0x80, 0x7F) : VR(0, 0xFF)));
		assert(VR().trunc(0xFF) == (isSigned!T ? VR(-0x80, 0x7F) : VR(0, 0xFF)));
		...
		assert(VR(176, 248).pad(0xFF) == (isSigned!T ? VR(-80, -8) : VR(176, 248)));
		assert(VR(-80, -8).pad(0xFF) == (isSigned!T ? VR(-80, -8) : VR(176, 248)));
		
		// Immaterializable ranges
		assert(VR(1, 158).pad(0xFF) == (isSigned!T ? VR(-128, 127) : VR(1, 158)));
		assert(VR(-8, 15).pad(0xFF) == (isSigned!T ? VR(-8, 15) : VR(0, 255)));
		assert(VR(5, 300).pad(0xFF) == (isSigned!T ? VR(-0x80, 0x7F) : VR(0, 0xFF)));
		...
		testAddSub(VR(1, smax), VR(smin, -1), VR(smin + 1, smax - 1), VR(2, -1));
		...
		testMul(VR(3, 3037000500), VR(7, 3037000500), VR(21, 9223372037000250000UL));
		...
		testMul(VR(5, 3037000500), VR(-12, 3037000500), VR(-36444006000, 9223372037000250000UL));
		
		// min > max on both side
		testMul(VR(-1, 5), VR(-7, 22), VR(-35, 110));
		testMul(VR(-11, 3037000500), VR(-8, 3037000500), VR(-33407005500, 9223372037000250000UL));
		...
		testDiv(VR(-27, 31), VR(5, 9), isSigned!T ? VR(-5, 6) : VR(0, T.max / 5));
		testDiv(VR(-23, 125), VR(89351496, 458963274), isSigned!T ? VR(0) : VR(0, (T.min - 23) / 89351496));
		...
		testDiv(VR(2, 1), VR(89351496, 458963274), VR(T.min / 89351496, T.max / 89351496));
		testDiv(VR(125, -23), VR(89351496, 458963274), VR(T.min / 89351496, T.max / 89351496));
		testDiv(VR(-12, -23), VR(89351496, 458963274), VR(T.min / 89351496, T.max / 89351496));
		...
		testRem(VR(-21, 16), VR(123, 456), isSigned!T ? VR(-21, 16) : VR(0, 455));
		...
		// degenerate numerator
		testRem(VR(125, -23), VR(210, 214), isSigned!T ? VR(-213, 213) : VR(0, 213));
		
		// modulo 0 elimination.
		testRem(VR(23), VR(0, 3), VR(0, 2));
		testRem(VR(121, 161), VR(-57, 52), isSigned!T ? VR(0, 56) : VR(0, 161));
		testRem(VR(-21, 34), VR(-17, 24), isSigned!T ? VR(-21, 23) : VR(0, T.max - 1));
		testRem(VR(34, 53), VR(-41, 36), isSigned!T ? VR(0, 40) : VR(0, 53));
		testRem(VR(-25, 42), VR(-13, 75), isSigned!T ? VR(-25, 42) : VR(0, T.max - 1));
		...
		// Signed shift
		testSRShift(VR(-35, -22), VR(1, 3), isSigned!T ? VR(-18, -3) : VR((umin - 35) >> 3, (umin - 22) >> 1));
		testSRShift(VR(-35, 22), VR(0, 3), isSigned!T ? VR(-35, 22) : VR((umin - 35) >> 3, 22));
		testSRShift(VR(-35, 22), VR(1, 3), isSigned!T ? VR(-18, 11) : VR(0, umax >> 1));
		...
		testSRShift(VR(23, 6), VR(1), isSigned!T ? VR(11, 3) : VR(0, umax >> 1));
		testSRShift(VR(23, -6), VR(0, 2), isSigned!T ? VR(5, -2) : VR(5, -6));
		testSRShift(VR(23, -6), VR(1, 2), isSigned!T ? VR(5, -2) : VR(5, (umin - 6) >> 1));
		...
		// Signed shift
		testURShift(VR(-35, -22), VR(1, 3), VR((umin - 35) >> 3, (umin - 22) >> 1));
